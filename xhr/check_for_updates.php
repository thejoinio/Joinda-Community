...
if ($f == 'check_for_updates') {
    ...
    if ($false == true) {
        $data['status']     = 400;
        $data['ERROR_NAME'] = 'It looks like you have renamed your themes, please rename them back to "joinda", "wonderful" to use the auto update system, otherwise please update your site manually.';
    ...
    if (Wo_CheckMainSession($hash_id) === true) {
        ...
        if (!empty($_GET['purchase_code'])) {
            ...
            $file          = file_get_contents("http://www.joinda.com/check_for_updates.php?code={$purchase_code}&version=$version&url=$siteurl", false, stream_context_create($arrContextOptions));
