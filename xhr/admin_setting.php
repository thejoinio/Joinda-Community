...
if ($f == 'admin_setting' AND (Wo_IsAdmin() || Wo_IsModerator())) {
    ...
    if ($s == 'verfiy_apps') {
        ...
        if (!empty($_POST['android_purchase_code'])) {
            ...
            $file         = file_get_contents("http://www.joinda.com/access_token.php?code={$android_code}&type=android", false, stream_context_create($arrContextOptions));
        ...
        if (!empty($_POST['windows_purchase_code'])) {
            ...
            $file         = file_get_contents("http://www.joinda.com/access_token.php?code={$windows_code}&type=windows_desktop", false, stream_context_create($arrContextOptions));
            $check        = json_decode($file, true);
            if (!empty($check['status'])) {
                if ($check['status'] == 'SUCCESS') {
        ...
        if (!empty($_POST['ios_purchase_code'])) {
            ...
            $file         = file_get_contents("http://www.joinda.com/access_token.php?code={$windows_code}&type=ios", false, stream_context_create($arrContextOptions));
            $check        = json_decode($file, true);
            if (!empty($check['status'])) {
                if ($check['status'] == 'SUCCESS') {
    ...
    if ($s == 'add_reaction') {
        $data['status']  = 400;
        $data['message'] = 'Please check your details';
        if (!empty($_FILES['joinda']) && !empty($_FILES['sunshine'])) {
            $joinda_image = '';
            $sunshine_image = '';
            $add            = false;
            $insert_data    = array();
            ...
            if ($add == true) {
                if (!empty($_FILES['joinda'])) {
                    $cover = getimagesize($_FILES["joinda"]["tmp_name"]);
                    if ($cover[0] > 48 || $cover[1] > 48) {
                        $data['status']  = 400;
                        $data['message'] = $error_icon . " joinda image size should not be more than 48x48 ";
                        header("Content-type: application/json");
                        echo json_encode($data);
                        exit();
                    }
                    $fileInfo = array(
                        'file' => $_FILES["joinda"]["tmp_name"],
                        'name' => $_FILES['joinda']['name'],
                        'size' => $_FILES["joinda"]["size"],
                        'type' => $_FILES["joinda"]["type"],
                        'types' => 'png'
                    );
                    $media    = Wo_ShareFile($fileInfo, true);
                    if (!empty($media) && !empty($media['filename'])) {
                        $joinda_image = $media['filename'];
                    } else {
                        $data['status']  = 400;
                        $data['message'] = $error_icon . " joinda image type must be png ";
                        header("Content-type: application/json");
                        echo json_encode($data);
                        exit();
                    }
                ...
                if (!empty($sunshine_image) && !empty($joinda_image)) {
                    $db->insert(T_REACTIONS_TYPES, array(
                        'name' => $id,
                        'joinda_icon' => $joinda_image,
    ...
    if ($s == 'delete_reaction') {
        ...
        if (!empty($_POST['id']) && is_numeric($_POST['id']) && $_POST['id'] > 0) {
            ...
            if ($id > 6 && !empty($reaction)) {
                $explode2       = @end(explode('.', $reaction->joinda_icon));
                $explode3       = @explode('.', $reaction->joinda_icon);
                $joinda_small = $explode3[0] . '_small.' . $explode2;
                ...
                if (file_exists($joinda_small)) {
                    @unlink(trim($joinda_small));
                } else if ($wo['config']['amazone_s3'] == 1 || $wo['config']['wasabi_storage'] == 1 || $wo['config']['ftp_upload'] == 1 || $wo['config']['backblaze_storage'] == 1) {
                    @Wo_DeleteFromToS3($joinda_small);
                }
                ...
                if (file_exists($reaction->joinda_icon)) {
                    @unlink(trim($reaction->joinda_icon));
                } else if ($wo['config']['amazone_s3'] == 1 || $wo['config']['wasabi_storage'] == 1 || $wo['config']['ftp_upload'] == 1 || $wo['config']['backblaze_storage'] == 1) {
                    @Wo_DeleteFromToS3($reaction->joinda_icon);
                }
    ...
    if ($s == 'get_reaction_form') {
        ...
        if (!empty($_POST['id']) && is_numeric($_POST['id']) && $_POST['id'] > 0) {
            ...
            if (!empty($reaction)) {
                ...
                $wo['joinda_icon'] = $reaction->joinda_icon;
        ...
        if (!empty($_POST['id']) && is_numeric($_POST['id']) && $_POST['id'] > 0) {
            ...
            if (!empty($reaction)) {
                ...
                if (!empty($_FILES['joinda'])) {
                    ...
                    $media    = Wo_ShareFile($fileInfo, true);
                    if (!empty($media) && !empty($media['filename'])) {
                        $update_data['joinda_icon'] = $media['filename'];
                    } else {
                        $data['status']  = 400;
                        $data['message'] = $error_icon . " joinda image type must be png ";
                        header("Content-type: application/json");
                ...
                if (!empty($_POST['joinda_to_use']) && $_POST['joinda_to_use'] == 1) {
                    $explode2       = @end(explode('.', $reaction->joinda_icon));
                    $explode3       = @explode('.', $reaction->joinda_icon);
                    $joinda_small = $explode3[0] . '_small.' . $explode2;
                    if (file_exists($reaction->joinda_icon)) {
