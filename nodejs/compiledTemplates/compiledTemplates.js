...
module.exports.chatListOwnerFalse = async (ctx, data, fromUser, nextId, hasHTML, sendable_message) => {
    ...
    ctx.reactions_types.forEach(element => {
        if (element.status == 1) {
            ...
            if (ctx.globalconfig['theme'] === "joinda") {
                r_img = '<img src="'+element.joinda_icon+'">';
                var matches = element.joinda_icon.match(/<[^<]+>/);
                if (matches) {
                    r_img = element.joinda_icon;
                }
                reactions_html += '<li style="'+first_text+'" class="reaction reaction-'+element.id+'" data-reaction="'+element.name+'" data-reaction-id="'+element.id+'" data-reaction-lang="'+element.name+'" data-post-id="'+nextId+'" onclick="Wo_RegisterMessageReaction(this,\''+element.joinda_small_icon+'\','+element.is_html+');">'+r_img+'</li>';
